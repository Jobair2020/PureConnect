...
class MyTableViewCell: UITableViewCell {
    ...
    var deleteButtonClickedCallback: (() -> Void)?
...
class TimelineViewController: UIViewController, UITableViewDelegate, UITableViewDataSource,  UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        //searchView.text = "hasib"

            // Simulate typing "esha" into searchView
            //searchView.sendActions(for: .editingChanged)
    ...
    func filterPosts(for searchText: String) {
            if searchText.isEmpty {
                ...
                // Use a case-insensitive search to find matching posts
                filteredPosts = listPost.filter { post in
                    return post.ownerNameText.lowercased().contains(searchText.lowercased())
                }
            }
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
            // Get the updated search text
            if let currentText = textField.text as NSString? {
                let updatedText = currentText.replacingCharacters(in: range, with: string)
                // Filter posts based on the updated search text
                filterPosts(for: updatedText)
            }
            return true
        }
    ...
    func handleDeleteButtonClicked(postID: String, cell: MyTableViewCell) {
        let likesRef = Database.database().reference().child("posts").child(postID)

        // Assuming you want to confirm the deletion with an alert
        ...
        alert.addAction(UIAlertAction(title: "Delete", style: .destructive, handler: { [weak self] _ in
            ...
            likesRef.removeValue { error, _ in
                if let error = error {
                    ...
                    self?.showAlert(title: "Error", message: "Failed to delete post. Please try again.")
                } else {
                    print("Post deleted successfully!")
                    // Show a success alert upon successful deletion
                    self?.showAlert(title: "Success", message: "Post deleted successfully!")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if post.ownerText == SharedData.shared.userID {
            cell.ownerView.text = "You"
        }
        if post.ownerText != SharedData.shared.userID {
        ...
        cell.deleteButtonClickedCallback = { [weak self] in
            self?.handleDeleteButtonClicked(postID: post.idText, cell: cell)
        }

        cell.deleteButton.isHidden = post.ownerText != SharedData.shared.userID
